...
| 번호 | 기능명 | 설명 | 담당자 |
|------|--------|------|--------|
| F-01 | 음성 녹음 | V키 핫키로 마이크 녹음 시작/종료 | 김민찬 |
| F-02 | 음성 재생 | TTS 오디오 재생 | 김민찬 |
| F-03 | 브라우저 실행 | Chrome 자동 실행 (CDP 활성화) | 김재환 |
| F-04 | 브라우저 제어 | MCP 도구로 웹 페이지 조작 | 김재환 |
| F-05 | 서버 통신 | WebSocket으로 AI 서버와 실시간 통신 | 김민찬 |
| F-06 | 세션 관리 | 현재 작업 상태 추적 | 김민찬 |
| F-07 | 상태 표시 | 최소 UI로 현재 상태 표시 | 김민찬 |
| F-08 | 텍스트 입력 (테스트용) | 콘솔에서 직접 텍스트 입력하여 AI 서버로 전송 | 김재환 |
