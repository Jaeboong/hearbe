...
| 번호 | 기능명 | 설명 | 상태 |
|------|--------|------|------|
| AI-04 | 의도 분석 | NLU 인텐트 분류 + 개체명 인식 | 세분화 완료 (2개 Story) |
| AI-05 | LLM 명령 생성 | LLM+Prompting 기반 MCP 명령 변환 | 세분화 완료 (3개 Story) ⚡ |
| AI-06 | 플로우 엔진 | 사이트별 쇼핑 플로우 단계 처리 | 세분화 완료 (7개 Story) |
| AI-07 | 대화형 응답 생성 | GPT 기반 친절한 응답 생성 | 단일 Story (5점) |
| AI-08 | 플로우 위임 판단 | 일반 vs 플로우 판단 로직 | 단일 Story (3점) |
| [Future] | RAG 고도화 | Vector DB 기반 명령 생성 | 백로그 (11점) 📦 |

> ⚡ LLM+Prompting 우선 접근, RAG는 필요시 도입
> 📦 백로그: 임베딩, Vector DB, RAG Pipeline (3개 작업)
