...
def _build_prompt(texts: List[str], product_type: ProductType, size_table: str = None) -> Dict[str, str]:
    ...
    if product_type in [ProductType.의류, ProductType.신발, ProductType.패션잡화]:
        size_instruction = (
            ...
            "   - 예: \"사이즈는 M(허리 34, 영덩이 67, 총장 103), L(허리 36, 영덩이 69, 총장 105)...\"\n"
            "   - SIZE 표의 **모든 항목**(허리, 엉덩이, 밑위, 허벅지, 밑단, 총장 등)을 빠짐없이 포함하세요.\n"
            "2. **(중요) 줄글로 나열된 표 해석 가이드**:\n"
            "   OCR 결과가 표 구조 없이 'M 34 67 35...' 처럼 줄글로 나열돼 있어도 당황하지 마세요.\n"
            "   - **패턴 인식**: '허리, 엉덩이, 총장' 같은 헤더가 먼저 나오고, 그 뒤에 'M', 'L' 같은 행(Row) 시작점이 나옵니다.\n"
            "   - **매핑**: 행 시작점(M) 뒤에 나오는 숫자들을 순서대로 헤더와 매칭하세요. (예: M -> 첫번째숫자(허리) -> 두번째숫자(엉덩이)...)\n"
            "3. **keywords**에도 \"사이즈\" 항목을 만들고, 동일한 내용을 포함하세요.\n"
            "4. 표에 있는 수치는 **절대 추측하지 말고** OCR 텍스트 그대로 옮기세요.\n"
        )
    ...
    instructions = (
        ...
        "   - 예: \"사이즈는 M(허리 34, 영덩이 67, 왓밀위 35, 뜻밀위 45, 허벅지 38, 밀단 24.5, 총장 103), L(허리 36, 영덩이 69, 왓밀위 36, 뜻밀위 46, 허벅지 39.5, 밀단 25.5, 총장 105), XL(허리 38, 영덩이 72, 왓밀위 37, 뜻밀위 47, 허벅지 41, 밀단 26.5, 총장 107), 2XL(허리 40, 영덩이 75, 왓밀위 38, 뜻밀위 48, 허벅지 42.5, 밀단 27.5, 총장 108)로 나와요.\"\n"
